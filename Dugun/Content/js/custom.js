...
function getItemsNumber(elem) {
    if (elem.length % 2 == 1) {
        $('.danisan-item-dummy').removeClass('hidden').addClass('fadeIn');
    }
}
...
;(function($, window, undefined){
    ...
    {	
        ...
        var isotopeContainer = $('#danisanlist');
        ...
        if (isotopeContainer.length) {
            ...
            $('.field-select').on('change', '#filter-difficulty, #filter-kilo, #filter-goal', function() {
                var $this = $(this).attr('id'),
		
                    difficulty =  $('#filter-difficulty').val(),
                    kilo =  $('#filter-kilo').val(),
                    goal =  $('#filter-goal').val(),
                    search = !difficulty && !kilo && !goal ? '*' : function() {
                        var item = $(this),
                            difficultyName = item.data('difficulty') ? item.data('difficulty') : '',
                            kiloName = item.data('kilo') ? item.data('kilo') : '',
                            goalName = item.data('goal') ? item.data('goal') : '';

                        if(difficultyName.match(new RegExp(difficulty)) && kiloName.match(new RegExp(kilo)) && goalName.match(new RegExp(goal))) {
                            if ($this == 'filter-difficulty') return difficultyName;
                            if ($this == 'filter-kilo') return kiloName;
                            if ($this == 'filter-goal') return goalName;
                        }
                    };

                $('#danisanlist').isotope({filter : search});

                // Hide 'Nothing Found' Message and the dummy Item
                $('.nothing-found, .danisan-item-dummy').removeClass('fadeIn').addClass('hidden');
            });
        ...
        public_vars.defaultColorsPalette = ['#A9D074','#7c38bc','#0e62c7','#fcd036','#4fcdfc','#00b19d','#ff6264','#f7aa47'];
...
function date(format, timestamp) {
	...
	'Ocak', 'Şubar', 'Mart', 'Nisan', 'Mayıs', 'Haziran',
...
jQuery(document).ready(function () {
    ...
    $("#target").click(function () {
        $('#liste').slimScroll({ scrollTo: '600px', animate: true });
       
    });

    $("#filter-difficulty").select2({
    ...
    $(function () {
        ...
        var wrapperH = $('#map').height();
      
        $('#map').css({ 'height': ($(window).height()-68) + 'px' });
        $('#liste').css({ 'height': ($(window).height()-105) + 'px' });
   
        $(window).resize(function () {
            var windowH = $(window).height();
            var wrapperH = $('#map').height();
            var differenceH = windowH - wrapperH;
            var newH = wrapperH + differenceH-68;
            var newHD = wrapperH + differenceH-105;
            var truecontentH = $('#page').height();
           
            $('#map').css('height', (newH) + 'px');
            $('#liste').css('height', (newHD) + 'px');
